...
# ============================ PARAMETROS ============================
CUSTO_POR_ORDEM = 0.0005                  # 0,05% por ordem; estresse realista: 0,20%
...
retorno_estrategia = (ret_invest + ret_caixa - giro * CUSTO_POR_ORDEM).dropna()

# ============================ 6) RESULTADOS (convencao unica: vs zero, igual ao B&H) ============================
...
# estresse de custo: o mesmo backtest a 20 bps por ordem
ret_estresse = (ret_invest + ret_caixa - giro * 0.0020).dropna()
sharpe_20bp  = ret_estresse.mean() / ret_estresse.std() * np.sqrt(DIAS_NO_ANO)

print("=== Rotacao v2: vol target no portfolio + rebalance semanal ===")
print(f"Periodo:          {retorno_estrategia.index[0].date()} a {retorno_estrategia.index[-1].date()}")
print(f"Sharpe (vs zero): {sharpe:.2f}   (liquido do CDI: {sharpe_cdi:.2f};  a 20 bps: {sharpe_20bp:.2f})")
